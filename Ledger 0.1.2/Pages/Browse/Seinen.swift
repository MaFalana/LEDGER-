...
struct Seinen: View
{
    ...
    var body: some View
    {
        HStack
        {
            Text("Seinen").fontWeight(.bold).padding()
            Spacer()
            NavigationLink(destination: viewMore(Genre: "Seinen", Data: Data).background(themeManager.selectedTheme.background).environmentObject(network) )
            {
