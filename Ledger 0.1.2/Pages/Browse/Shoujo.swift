...
struct Shoujo: View
{
    ...
    var body: some View
    {
        HStack
        {
            Text("Shoujo").fontWeight(.bold).padding()
            Spacer()
            NavigationLink(destination: viewMore(Genre: "Shoujo", Data: Data).background(themeManager.selectedTheme.background).environmentObject(network) )
            {
                
                Text("View More").padding()
                //Image(systemName: "chevron.right").padding().foregroundColor(.theme.accent).fontWeight(.bold)
            }
        }
